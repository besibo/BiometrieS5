...
\pandocbounded{\includegraphics[keepaspectratio]{images/Grille.jpg}}
...
\pandocbounded{\includegraphics[keepaspectratio]{01-OneSampleTests_files/figure-pdf/unnamed-chunk-14-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{01-OneSampleTests_files/figure-pdf/unnamed-chunk-15-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{01-OneSampleTests_files/figure-pdf/unnamed-chunk-18-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{01-OneSampleTests_files/figure-pdf/unnamed-chunk-19-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{01-OneSampleTests_files/figure-pdf/unnamed-chunk-20-1.pdf}}
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{images/Blackbirds.png}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-TwoSamplePairedTests_files/figure-pdf/unnamed-chunk-11-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-TwoSamplePairedTests_files/figure-pdf/unnamed-chunk-12-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{02-TwoSamplePairedTests_files/figure-pdf/unnamed-chunk-17-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/HornAlive.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/HornDead.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/HornPredator.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-TwoSampleTests_files/figure-pdf/unnamed-chunk-12-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-TwoSampleTests_files/figure-pdf/unnamed-chunk-13-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{03-TwoSampleTests_files/figure-pdf/unnamed-chunk-32-1.pdf}}
...
\item
  Ajuster un modèle simple pour séparer la survie (\(\phi\)) de la
  probabilité de détection (\(p\)).
\item
  Construire une matrice de Leslie et simuler la dynamique d'une
  population structurée en stades (cinétique).
\item
  Évaluer la sensibilité d'une espèce longévive, en lien avec la
  ``spirale de l'extinction'' vue en cours.
\end{itemize}
...
dans les packages R que nous allons utiliser.

Vous avez juste besoin de charger trois bibliothèques (installez-les via
la console de RStudio si ce n'est pas encore fait avec
...
Sur le terrain, la réalité est complexe : un animal vivant n'est pas
toujours observé. Comme vu en CM avec les comptages de mésanges en
fôret, la probabilité de détection (\(p\)) est presque toujours
inférieure à 1.
...
{\centering \includegraphics[width=0.5\linewidth,height=\textheight,keepaspectratio]{images/cincle.jpg}
...
calcul de l'algorithme d'optimisation dans la console de RStudio.
Regardez le tableau final produit par \texttt{predict()}.

\begin{itemize}
\item
  La probabilité de survie moyenne d'une année sur l'autre (\(\Phi\))
  est d'environ \textbf{0.56 (56\%)}.
\item
  La probabilité de détection (\(p\)) est d'environ \textbf{0.90
  (90\%)}. Cela signifie que les chercheurs ont raté en moyenne 10\% des
  oiseaux vivants à chaque sortie.
\end{itemize}
\end{quote}

Une valeur moyenne globale masque parfois des disparités biologiques
fondamentales. En écologie des populations, les rôles écologiques,
l'effort reproducteur ou les comportements territoriaux diffèrent
souvent entre mâles et femelles. Vérifions si la survie (\(\Phi\)) est
équivalente entre les sexes en intégrant la covariable \texttt{sex} dans
les formules du modèle :

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mod\_sexe }\OtherTok{\textless{}{-}} \FunctionTok{crm}\NormalTok{(dipper, }\AttributeTok{model =} \StringTok{"cjs"}\NormalTok{, }
                \AttributeTok{model.parameters =} \FunctionTok{list}\NormalTok{(}\AttributeTok{Phi =} \FunctionTok{list}\NormalTok{(}\AttributeTok{formula =} \SpecialCharTok{\textasciitilde{}}\NormalTok{sex), }
                                        \AttributeTok{p   =} \FunctionTok{list}\NormalTok{(}\AttributeTok{formula =} \SpecialCharTok{\textasciitilde{}}\DecValTok{1}\NormalTok{)))                                   }
\end{Highlighting}
\end{Shaded}

Utilisez ensuite la fonction \texttt{predict} pour afficher l'estimation
des paramètres du modèle :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{predict}\NormalTok{(mod\_sexe)   }
\end{Highlighting}
\end{Shaded}
...
\textbf{Questions d'interprétation :}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Survie différentielle et probabilités cumulées :}

  \begin{itemize}
  \tightlist
  \item
    Quelle est la différence annuelle brute de survie entre mâles et
    femelles ?
  \item
    Calculez la probabilité pour un individu d'être encore vivant au
    bout de 5 ans dans chaque sexe (\(\Phi^5\)). Que constatez-vous sur
    l'écart relatif entre mâles et femelles après quelques années ?
  \end{itemize}
\item
  \textbf{Biais de détection (\(p\)) :}

  \begin{itemize}
  \tightlist
  \item
    Si un écologue de terrain n'utilisait pas de modèle CJS et calculait
    simplement le ratio des oiseaux revus d'une année sur l'autre,
    quelle valeur approchée de survie obtiendrait-il (\(\Phi \times p\))
    ? Pourquoi confondre « non-détecté » et « mort » est-il risqué ?
  \item
    Comparez la probabilité de détection (\(p\)) estimée pour les
    femelles et pour les mâles (changer le code pour vous-même).
    Constatez-vous une différence notable ? En vous renseignant
    brièvement sur l'aspect visuel (plumage) et les comportements
    territoriaux du Cincle plongeur (\emph{Cinclus cinclus}), quelles
    caractéristiques biologiques de cette espèce expliquent logiquement
    cette divergence (ou similitude) de détectabilité pour l'observateur
    ?
  \end{itemize}
\end{enumerate}
...
Le détective vient d'estimer la survie (\(\Phi\)) et la détection
(\(p\)). Mais la survie n'est qu'une pièce du puzzle.

Sur le terrain, les écologues déploient une vaste panoplie de modèles
statistiques (que nous ne verrons pas dans ce TP) pour estimer
l'ensemble des traits d'histoire de vie d'une population, par exemple :

\begin{itemize}
\item
  \textbf{La fécondité (\(F\)) :} estimée par le suivi des pontes, des
  nichées ou par des modèles de comptage (ex. modèles de Poisson ou
  binomial négatif).
\item
  \textbf{L'âge à la première reproduction (\(E\)) :} estimé via des
  modèles multi-états ou des suivis longitudinaux d'individus d'âge
  connu.
\item
  \textbf{Les probabilités de transition / maturation (\(\psi\)) :}
  estimées via des modèles de Capture-Marquage-Recapture multi-sites ou
  multi-états (ex. passage du statut ``juvénile'' à ``adulte'').
\end{itemize}

Une fois ces paramètres estimés par les « détectives », ils ne restent
pas de simples chiffres isolés dans un tableau : \textbf{l'architecte
s'en empare pour construire la matrice de projection}.

Chaque case d'une matrice démographique (Matrice de Leslie ou de
Lefkovitch) correspond précisément à l'un de ces paramètres vitaux
estimés sur le terrain. Nous changeons maintenant d'espèce et d'échelle
: nous allons assembler ces briques pour simuler le devenir de la
population et orienter les décisions de gestion.
...
{\centering \includegraphics[width=0.65\linewidth,height=\textheight,keepaspectratio]{images/orca.jpg}
...
juvénile, adulte mature, et post-reproductive (ménopause).
...
\item
  La \textbf{première ligne} contient la fécondité (seules les adultes
  matures se reproduisent, avec un taux de 0.04).
\item
  Les \textbf{autres lignes} contiennent les probabilités de survie et
  de passage d'un stade à l'autre.
\end{itemize}
\end{quote}
...
\pandocbounded{\includegraphics[keepaspectratio]{04-Structure_files/figure-pdf/projection-whale-1.pdf}}

\begin{quote}
\textbf{Observation :} Regardez les 10 premières années sur le
graphique. Les courbes zigzaguent ! C'est ce qu'on appelle
\textbf{l'inertie démographique} vue en CM. La population met du temps à
se ``rééquilibrer'' par rapport à notre structure de départ
artificielle. Ensuite, les courbes deviennent parallèles : la population
a atteint sa distribution stable.
\end{quote}
...
\FunctionTok{image2}\NormalTok{(sensibilite\_orque, }
       \AttributeTok{log =} \ConstantTok{FALSE}\NormalTok{, }
       \AttributeTok{col =} \FunctionTok{c}\NormalTok{(}\StringTok{"white"}\NormalTok{, }\FunctionTok{rev}\NormalTok{(}\FunctionTok{heat.colors}\NormalTok{(}\DecValTok{23}\NormalTok{))),}
       \AttributeTok{labels =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{),}
       \AttributeTok{mar =} \FunctionTok{c}\NormalTok{(}\FloatTok{4.5}\NormalTok{, }\DecValTok{5}\NormalTok{, }\FloatTok{3.5}\NormalTok{, }\DecValTok{1}\NormalTok{),}
       \AttributeTok{box.offset =} \FloatTok{0.1}\NormalTok{)}

\CommentTok{\# Le haut étant dégagé, le titre se place sans conflit}
\FunctionTok{title}\NormalTok{(}\StringTok{"Quels paramètres influencent le plus la croissance ?"}\NormalTok{, }\AttributeTok{line =} \FloatTok{1.8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{04-Structure_files/figure-pdf/sens-whale-1.pdf}}

\begin{quote}
\textbf{Règle de lecture d'une matrice démographique :}
\end{quote}
...
\tightlist
\item
  \textbf{En colonne (\(X\), temps \(t\)) :} le stade de \textbf{départ}
  de l'individu.
\item
  \textbf{En ligne (\(Y\), temps \(t+1\)) :} le stade d'\textbf{arrivée}
  l'année suivante.
\end{itemize}

\emph{Chaque cellule se lit donc : « De la colonne vers la ligne ».}
\end{quote}
...
\tightlist
\item
  La \textbf{première ligne} correspond aux naissances (vers le stade
  \emph{yearling}). Seules les femelles matures s'y projettent
  (fécondité de 0.04).
\item
  La \textbf{diagonale} correspond aux individus qui survivent et
  restent dans le même stade d'une année sur l'autre (ex. adulte restant
  adulte : 0.95).
\item
  La \textbf{sous-diagonale} correspond aux transitions de croissance
  (ex. juvénile devenant mature : 0.07).
\end{itemize}
\end{quote}

\begin{quote}
\textbf{Guide d'interprétation du graphique :}

\begin{itemize}
\tightlist
\item
  \textbf{Comment lire la grille :}
  ...
  \item
    Remontez verticalement vers le stade d'arrivée sur l'axe de gauche
    (\(t+1\)).
  \end{enumerate}
\item
  \textbf{Distinction essentielle : Réalité biologique vs Sensibilité
  mathématique :}

  \begin{itemize}
  \tightlist
  \item
    \textbf{Transition réelle {[}juvenile \(\to\) mature = 0.567{]} :}
    Forte sensibilité. Elle représente la maturation (le passage des
    grands jeunes vers le stock reproducteur).
  \item
    \textbf{Transition réelle {[}mature \(\to\) mature = 0.579{]} :}
    Très forte sensibilité. C'est le maintien des adultes reproducteurs
    déjà en place, pilier de la population.
  \item
    \textbf{Cas mathématiques théoriques {[}postreprod \(\to\) mature =
    0.581{]} ou {[}mature \(\to\) juvenile = 0.387{]} :} La sensibilité
    brute calcule la dérivée mathématique même pour des cases nulles
    dans la nature. Une orque ménopausée ne redevient jamais mature, et
    un adulte ne rajeunit jamais en juvénile (\(A_{ij} = 0\)). En
    biologie de la conservation, \textbf{on ne prend en compte que les
    cases correspondant à des transitions réelles}.
  \end{itemize}
\end{itemize}
\end{quote}
...
{\centering \includegraphics[width=0.55\linewidth,height=\textheight,keepaspectratio]{images/tortoise.jpg}
...
\textbf{Contexte historique et gestionnaire (Doak et al., 1994) :} Dans
les années 1990, l'armée américaine souhaitait agrandir la base
militaire de Fort Irwin, menaçant de détruire jusqu'à 13\% de la
population restante de Tortues du désert en Californie. Pour évaluer la
viabilité de l'espèce, les écologues Doak, Kareiva et Klepetka (1994)
ont construit des matrices structurées en 8 stades de taille.

Sur le terrain, la survie des adultes était connue, mais \textbf{le
recrutement des jeunes tortues était quasi impossible à mesurer} car les
nouveau-nés sont minuscules et passent inaperçus sous les buissons. Les
auteurs ont donc testé 4 scénarios de fécondité (production annuelle de
jeunes femelles par adulte) :

\begin{itemize}
\tightlist
\item
  \textbf{\texttt{low} :} Comptage brut des jeunes observés (très
  fortement sous-estimé par la détection imparfaite).
\item
  \textbf{\texttt{med.low} :} Estimation corrigée (multipliée
  arbitrairement par 10 pour compenser le biais d'observation).
\item
  \textbf{\texttt{med.high} et \texttt{high} :} Estimations basées sur
  une population témoin particulièrement saine et féconde (site de
  Goffs), représentatives d'années humides exceptionnelles.
\end{itemize}

Ces matrices sont stockées dans la liste \texttt{tortoise} du package
\texttt{popbio}.

\textbf{Votre mission :} Exécutez le code ci-dessous étape par étape
dans RStudio et répondez aux questions sur votre brouillon.
...
\textbf{Lecture biologique} 8 stades de développement : de l'éclosion
(yearling) aux très grands individus (adult2). La reproduction ne débute
qu'au stade subadulte (subadult, carapace \(>180\) mm), ce qui prend
entre 15 et 20 ans dans le désert !

\textbf{2. Viabilité de la population} Calculez le taux de croissance
(\(\lambda\)) du scénario \texttt{med.high}. \emph{Question : Cette
population est-elle en croissance ou en déclin dans le cadre de ce
scénario?}
...
\emph{Question : Que remarquez-vous sur la valeur de \(\lambda\), même
sous le scénario le plus optimiste (high) ? Si une réserve finance des
couveuses artificielles ou des programmes de reproduction en captivité,
cela suffira-t-il à sauver l'espèce ?}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Astuce : remplacez "???" par le nom de la matrice "haute" ("high")}
\NormalTok{sens\_tortue }\OtherTok{\textless{}{-}} \FunctionTok{sensitivity}\NormalTok{(tortoise[[}\StringTok{"???"}\NormalTok{]])}

\FunctionTok{image2}\NormalTok{(sens\_tortue, }
       \AttributeTok{log =} \ConstantTok{FALSE}\NormalTok{, }
       \AttributeTok{col =} \FunctionTok{c}\NormalTok{(}\StringTok{"white"}\NormalTok{, }\FunctionTok{rev}\NormalTok{(}\FunctionTok{heat.colors}\NormalTok{(}\DecValTok{23}\NormalTok{))),}
       \AttributeTok{labels =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{),}
       \AttributeTok{mar =} \FunctionTok{c}\NormalTok{(}\FloatTok{4.5}\NormalTok{, }\DecValTok{5}\NormalTok{, }\FloatTok{3.5}\NormalTok{, }\DecValTok{1}\NormalTok{),}
...
\FunctionTok{title}\NormalTok{(}\StringTok{"Sensibilité {-} Tortue du désert"}\NormalTok{, }\AttributeTok{line =} \FloatTok{1.8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\emph{Question : Comparez la sensibilité de la reproduction (ligne 1) à
celle de la survie des adultes (diagonale adult1). À l'époque, les
gestionnaires voulaient abattre les corbeaux qui mangeaient les jeunes
tortues. À la lumière de cette matrice, était-ce la stratégie la plus
efficace ?}

\textbf{5. Boucler la boucle : Du terrain au modèle de projection}

Vous êtes chargé de diagnostiquer la viabilité de la population de
Cincles plongeurs que vous avez analysée au début du TP :

\begin{itemize}
\item
  Les suivis de nids indiquent un recrutement de \textbf{1.3 jeune
  femelle par adulte} et une survie juvénile en première année de
  \textbf{0.35}.
\item
  Vous disposez de la survie adulte réelle estimée par votre modèle CMR
  (\textbf{\(\Phi = 0.56\)}).
\end{itemize}

Construisez la matrice de Leslie correspondante (\(2 \times 2\) :
Juvéniles, Adultes) :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 1. Matrice avec la survie estimée par le modèle CJS (Phi = 0.56)}
\NormalTok{mat\_cincle }\OtherTok{\textless{}{-}} \FunctionTok{matrix}\NormalTok{(}\FunctionTok{c}\NormalTok{(???,    ???, }
\NormalTok{                       ???,  ???), }
                     \AttributeTok{nrow =} \DecValTok{2}\NormalTok{, }\AttributeTok{byrow =} \ConstantTok{TRUE}\NormalTok{)}
\FunctionTok{colnames}\NormalTok{(mat\_cincle) }\OtherTok{\textless{}{-}} \FunctionTok{rownames}\NormalTok{(mat\_cincle) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Juvénile"}\NormalTok{, }\StringTok{"Adulte"}\NormalTok{)}

\CommentTok{\# Taux de croissance asymptotique}
\FunctionTok{lambda}\NormalTok{(mat\_cincle)}
\end{Highlighting}
\end{Shaded}

\emph{Question 1 : La population de cincle est-elle viable avec cette
survie estimée (𝜆 \textgreater1)?}

\emph{Question 2 : Si un observateur débutant n'avait pas corrigé la
détection imparfaite (𝑝 =0.90) et avait estimé la survie brute apparente
à 0.50 au lieu de 0.56, quelle aurait été sa conclusion sur l'avenir de
la population ?}

\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomrule=.15mm, bottomtitle=1mm, breakable, colback=white, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, left=2mm, leftrule=.75mm, opacityback=0, opacitybacktitle=0.6, rightrule=.15mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Défi : Modéliser le décrochage des trajectoires temporelles {[}Optionnel
- Niveau Avancé{]}}, titlerule=0mm, toprule=.15mm, toptitle=1mm]

Un écart de \(\lambda\) abstrait (1.010 vs 0.969) ne parle pas toujours
aux décideurs. Pour convaincre un gestionnaire, rien ne vaut une courbe
de projection comparée.

\textbf{Votre défi :}\\
À l'aide des fonctions vues plus haut pour l'Orque
(\texttt{pop.projection}), construisez un script simulant l'évolution
des deux populations sur 30 ans à partir d'un stock de départ identique
de 100 oiseaux (50 juvéniles et 50 adultes).
...
\tightlist
\item
  Définissez la durée (\texttt{duree\ \textless{}-\ 30}) et le vecteur
  initial \texttt{n\_depart}.
\item
  Projetez les deux matrices (\texttt{mat\_cincle\_reelle} et
  \texttt{mat\_cincle\_naif}) sur cette période. \emph{(Indice : pour
  couvrir les pas de temps de 0 à 30, demandez
  \texttt{iterations\ =\ duree\ +\ 1})}.
\item
  Extrayez l'abondance totale à chaque pas de temps via
  \texttt{colSums(proj\$stage.vectors)} et assemblez-les dans un tableau
  (\texttt{tibble}) contenant une colonne \texttt{Annee} (de 0 à 30),
  une colonne \texttt{Reel} et une colonne \texttt{Naif}.
\item
  Passez ce tableau au format long avec \texttt{pivot\_longer()} pour
  tracer les deux trajectoires sur un même graphique \texttt{ggplot2}.
\end{enumerate}

\emph{(Besoin d'un coup de pouce ? Complétez le canevas ci-dessous en
remplaçant les \texttt{???})} :

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# 1. Paramètres initiaux}
\NormalTok{n\_depart }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\DecValTok{50}\NormalTok{, }\DecValTok{50}\NormalTok{)}
\NormalTok{duree    }\OtherTok{\textless{}{-}} \DecValTok{30}

\CommentTok{\# 2. Projections matricielles}
\NormalTok{proj\_reel }\OtherTok{\textless{}{-}} \FunctionTok{pop.projection}\NormalTok{(}\AttributeTok{A =}\NormalTok{ mat\_cincle\_reelle, }\AttributeTok{n =}\NormalTok{ n\_depart, }\AttributeTok{iterations =}\NormalTok{ ???)}
\NormalTok{proj\_naif }\OtherTok{\textless{}{-}} \FunctionTok{pop.projection}\NormalTok{(}\AttributeTok{A =}\NormalTok{ mat\_cincle\_naif,   }\AttributeTok{n =}\NormalTok{ n\_depart, }\AttributeTok{iterations =}\NormalTok{ ???)}

\CommentTok{\# 3. Structuration des résultats}
\NormalTok{df\_comparaison }\OtherTok{\textless{}{-}} \FunctionTok{tibble}\NormalTok{(}
  \AttributeTok{Annee =} \DecValTok{0}\SpecialCharTok{:}\NormalTok{duree,}
  \AttributeTok{Reel  =} \FunctionTok{colSums}\NormalTok{(proj\_reel}\SpecialCharTok{$}\NormalTok{???),}
  \AttributeTok{Naif  =} \FunctionTok{colSums}\NormalTok{(proj\_naif}\SpecialCharTok{$}\NormalTok{???)}
\NormalTok{) }\SpecialCharTok{|\textgreater{}} 
  \FunctionTok{pivot\_longer}\NormalTok{(}\AttributeTok{cols =} \FunctionTok{c}\NormalTok{(Reel, Naif), }\AttributeTok{names\_to =} \StringTok{"Scenario"}\NormalTok{, }\AttributeTok{values\_to =} \StringTok{"Effectif\_Total"}\NormalTok{)}

\CommentTok{\# 4. Tracé ggplot2}
\FunctionTok{ggplot}\NormalTok{(df\_comparaison, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Annee, }\AttributeTok{y =}\NormalTok{ Effectif\_Total, }\AttributeTok{color =}\NormalTok{ Scenario, }\AttributeTok{linetype =}\NormalTok{ Scenario)) }\SpecialCharTok{+}
  \FunctionTok{geom\_line}\NormalTok{(}\AttributeTok{linewidth =} \FloatTok{1.2}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{size =} \DecValTok{2}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_hline}\NormalTok{(}\AttributeTok{yintercept =} \FunctionTok{sum}\NormalTok{(n\_depart), }\AttributeTok{linetype =} \StringTok{"dotted"}\NormalTok{, }\AttributeTok{color =} \StringTok{"grey40"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{scale\_color\_manual}\NormalTok{(}
    \AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"Reel"} \OtherTok{=} \StringTok{"forestgreen"}\NormalTok{, }\StringTok{"Naif"} \OtherTok{=} \StringTok{"firebrick"}\NormalTok{),}
    \AttributeTok{labels =} \FunctionTok{c}\NormalTok{(}\StringTok{"Comptage brut naïf (Phi = 0.50, faux déclin)"}\NormalTok{, }\StringTok{"Modèle CJS corrigé (Phi = 0.56, viable)"}\NormalTok{)}
\NormalTok{  ) }\SpecialCharTok{+}
  \FunctionTok{scale\_linetype\_manual}\NormalTok{(}
    \AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"Reel"} \OtherTok{=} \StringTok{"solid"}\NormalTok{, }\StringTok{"Naif"} \OtherTok{=} \StringTok{"dashed"}\NormalTok{),}
    \AttributeTok{labels =} \FunctionTok{c}\NormalTok{(}\StringTok{"Comptage brut naïf (Phi = 0.50, faux déclin)"}\NormalTok{, }\StringTok{"Modèle CJS corrigé (Phi = 0.56, viable)"}\NormalTok{)}
\NormalTok{  ) }\SpecialCharTok{+}
  \FunctionTok{scale\_x\_continuous}\NormalTok{(}\AttributeTok{breaks =} \FunctionTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{, duree, }\AttributeTok{by =} \DecValTok{5}\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}
    \AttributeTok{title =} \StringTok{"Impact de la correction CMR sur la trajectoire projetée du Cincle"}\NormalTok{,}
    \AttributeTok{subtitle =} \StringTok{"Divergence exponentielle entre une population viable et un faux déclin"}\NormalTok{,}
    \AttributeTok{x =} \StringTok{"Années de projection"}\NormalTok{,}
    \AttributeTok{y =} \StringTok{"Effectif total d\textquotesingle{}oiseaux"}\NormalTok{,}
    \AttributeTok{color =} \StringTok{"Approche"}\NormalTok{,}
    \AttributeTok{linetype =} \StringTok{"Approche"}
\NormalTok{  ) }\SpecialCharTok{+}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/Nassarius.png}}
...
\NormalTok{Nas\_01 }\OtherTok{\textless{}{-}}\NormalTok{ Nassarius }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{filter}\NormalTok{(date }\SpecialCharTok{==} \StringTok{"2010{-}03"}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{05-Cohortes_files/figure-pdf/fig-histo1-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{05-Cohortes_files/figure-pdf/fig-histo2-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{05-Cohortes_files/figure-pdf/fig-mixplot-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{05-Cohortes_files/figure-pdf/fig-ajustement-1.pdf}}
...
\begin{quote}
\textbf{De l'importance du choix des valeurs initiales}
...
\item
  ...
  attentivement
\item
  déterminer des valeurs plus appropriées\footnotemark{} pour la
  fonction \texttt{mixparam()}
\item
...
\item
  ...
  réaliser le suivi\footnotemark{}
\end{enumerate}
...
totale\footnotemark{}, tout ce dont on a besoin pour connaitre
l'abondance de la cohorte la plus jeune est :
...
\pandocbounded{\includegraphics[keepaspectratio]{05-Cohortes_files/figure-pdf/fig-croiss-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{05-Cohortes_files/figure-pdf/fig-survie-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{05-Cohortes_files/figure-pdf/fig-allen-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{05-Cohortes_files/figure-pdf/fig-biomasse-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{06-DynamicSystems_files/figure-pdf/fig-mort-1.pdf}}
...
\begin{quote}
\textbf{Pour comprendre ce modèle\ldots{}}
...
\pandocbounded{\includegraphics[keepaspectratio]{06-DynamicSystems_files/figure-pdf/fig-crois-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{06-DynamicSystems_files/figure-pdf/fig-logist-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{06-DynamicSystems_files/figure-pdf/fig-lotka-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{06-DynamicSystems_files/figure-pdf/unnamed-chunk-11-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{06-DynamicSystems_files/figure-pdf/fig-volterra-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{06-DynamicSystems_files/figure-pdf/fig-compet_strong-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{06-DynamicSystems_files/figure-pdf/fig-compet-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{06-DynamicSystems_files/figure-pdf/fig-3spe-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{07-MarkovChains_files/figure-pdf/fig-markov-1.pdf}}
...
\begin{quote}
\textbf{Pour comprendre ce système\ldots{}}
...
\footnotetext{ajoutez ou retirez une cohorte si besoin, et choisissez
des valeurs plus proches des pics observés}

\footnotetext{on sait déjà que cette taille vaut 3.9 millimètres en mars
2010.}

\footnotetext{c'est ce que nous dit la valeur de \texttt{pi} de la
première cohorte dans le tableau \texttt{res\_01\$parameters} :}
